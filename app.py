...
# # )
# # st.image("https://images.unsplash.com/photo-1513258496099-48168024aec0", use_column_width=True)
...
# # # Document loader and embedder with chunking
...
# # if uploaded_files:
# #     st.session_state["vector_store"] = load_and_embed(uploaded_files)
...
# # def get_rag_response(query, chat_history, vector_store, temperature, prompt):
...
# #     st.markdown(f"<div style='margin-bottom: 10px;'><b>You:</b> {user_q}</div>", unsafe_allow_html=True)
# #     st.markdown(f"<div style='margin-bottom: 20px;'><b>StudyMate AI:</b> {ai_a}</div>", unsafe_allow_html=True)
...
# # When PDFs are uploaded, process and embed
# if uploaded_files:
#     st.session_state["vector_store"] = load_and_embed(uploaded_files)
#     st.success("Documents uploaded and knowledge base created!")
...
# def get_rag_response(query, chat_history, vector_store, temperature, prompt):
#     # retrieve all relevant chunks above a similarity score threshold (not top-k)
...
# # Use a separate key for input bar to manage reactivity and proper clearing
# # Chat history display loop (as before)

# with st.form("chat_form", clear_on_submit=True):
#     user_input = st.text_input("Ask me anything about your studies...", key="input_box")
...
# # ----- MAIN LOGIC -----
# # Vector store re-computed every time (stateless, per upload+question)
# vector_store = None
# if uploaded_files:
...
#             st.markdown(f"<div style='margin-bottom: 20px;'><b>StudyMate AI:</b> {answer}</div>", unsafe_allow_html=True)
